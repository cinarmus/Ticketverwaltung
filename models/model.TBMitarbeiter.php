...
//Konstanten
const SQL_INSERT='INSERT INTO TBMitarbeiter (_User_uid, `Name_Vorname` , `Name_Nachname` , `Abteilung` , `Adresse_Straße` , `Adresse_PLZ` , `Adresse_Ort` ) VALUES (?, ?, ?, ?, ?, ?, ?)';
const SQL_UPDATE='UPDATE TBMitarbeiter SET _User_uid=?, `Name_Vorname` =?, `Name_Nachname` =?, `Abteilung` =?, `Adresse_Straße` =?, `Adresse_PLZ` =?, `Adresse_Ort` =? where id=?';
const SQL_SELECT_PK='SELECT TBMitarbeiter.`c_ts` as `c_ts`, TBMitarbeiter.`m_ts` as `m_ts`, TBMitarbeiter.`id` as `id`, TBMitarbeiter._User_uid as _User_uid, `TBMitarbeiter`.`Name_Vorname` as `Name_Vorname`, `TBMitarbeiter`.`Name_Nachname` as `Name_Nachname`, `AbteilungT0`.`literal` as `Abteilung_literal`, `TBMitarbeiter`.`Abteilung` as `Abteilung`, `TBMitarbeiter`.`Adresse_Straße` as `Adresse_Straße`, `TBMitarbeiter`.`Adresse_PLZ` as `Adresse_PLZ`, `TBMitarbeiter`.`Adresse_Ort` as `Adresse_Ort` from TBMitarbeiter left join `AbteilungT` as AbteilungT0 on `TBMitarbeiter`.`Abteilung` = `AbteilungT0`.`id`  where TBMitarbeiter.`id` = ?';
